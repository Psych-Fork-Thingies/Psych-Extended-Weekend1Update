	...
	public static function getModSetting(saveTag:String, ?modName:String = null)
	{
		...
		if(FileSystem.exists(path))
		{
			if(settings == null || !settings.exists(saveTag))
			{
				...
				try
				{
					//StateLua.luaTrace('getModSetting: Trying to find default value for "$saveTag" in Mod: "$modName"');
					...
					for (i in 0...parsedJson.length)
					{
						...
						if(sub != null && sub.save != null && !settings.exists(sub.save))
						{
							if(sub.type != 'keybind' && sub.type != 'key')
							{
								if(sub.value != null)
								{
									//StateLua.luaTrace('getModSetting: Found unsaved value "${sub.save}" in Mod: "$modName"');
									settings.set(sub.save, sub.value);
								}
							}
							else
							{
								//StateLua.luaTrace('getModSetting: Found unsaved keybind "${sub.save}" in Mod: "$modName"');
