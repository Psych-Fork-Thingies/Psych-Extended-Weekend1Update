 ...
 */
class VirtualButton extends TypedVirtualButton<FlxSprite>
{
	/**
	...
	**/
	...
	/**
	 * Creates a new `VirtualButton` object
...
@:generic
#end
class TypedVirtualButton<T:FlxSprite> extends FlxSprite implements IFlxInput
{
	/**
	...
	/**
 	 * Shows the current state of the button, either `VirtualButton.NORMAL`,
 	 * `VirtualButton.HIGHLIGHT` or `VirtualButton.PRESSED`.
 	 */
	...
	public function new(X:Float = 0, Y:Float = 0):Void
	{
		...
		onOver = new VirtualButtonEvent();
		onOut = new VirtualButtonEvent();

		status = multiTouch ? VirtualButton.NORMAL : VirtualButton.HIGHLIGHT;
	...
	function updateButton():Void
	{
		...
		if (status != VirtualButton.NORMAL && (!overlapFound || (currentInput != null && currentInput.justReleased)))
			onOutHandler();
	}
	...
	function updateStatus(input:IFlxInput):Void
	{
		...
		else if (status == VirtualButton.NORMAL)
		{
	...
	{
		status = VirtualButton.HIGHLIGHT;
...
/** 
 * Helper function for `VirtualButton` which handles its events.
 */
private class VirtualButtonEvent implements IFlxDestroyable
{
	/**
