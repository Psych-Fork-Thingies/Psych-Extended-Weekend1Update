...
import StrumNote;
import Note;
import objects.Alphabet; //use 0.7x's Alphabet
...
class NotesColorSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		var text:Alphabet = new Alphabet(#if mobile 44 #else 50 #end, 86, #if mobile 'PRESS' #else 'CTRL' #end, false);
	...
	function updateTip()
	{
		#if !mobile tipTxt.text = 'Hold Shift + Press RESET key to fully reset the selected Note.'; #end
	}
	...
	public function spawnNotes()
	{
		...
		if (onPixel) PlayState.isPixelStage = true;
